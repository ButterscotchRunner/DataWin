**Progress:** 118/233 (50.64377682403433%)

| GML Variable | Implemented in Butterscotch? | Added In | Removed In |
| - | - | - | - |
| `alarm` | ✅ | GM:S 1.0.98 |  |
| `argument` | ✅ | GM:S 1.0.98 |  |
| `argument0` | ✅ | GM:S 1.0.98 |  |
| `argument1` | ✅ | GM:S 1.0.98 |  |
| `argument10` | ✅ | GM:S 1.0.98 |  |
| `argument11` | ✅ | GM:S 1.0.98 |  |
| `argument12` | ✅ | GM:S 1.0.98 |  |
| `argument13` | ✅ | GM:S 1.0.98 |  |
| `argument14` | ✅ | GM:S 1.0.98 |  |
| `argument15` | ✅ | GM:S 1.0.98 |  |
| `argument2` | ✅ | GM:S 1.0.98 |  |
| `argument3` | ✅ | GM:S 1.0.98 |  |
| `argument4` | ✅ | GM:S 1.0.98 |  |
| `argument5` | ✅ | GM:S 1.0.98 |  |
| `argument6` | ✅ | GM:S 1.0.98 |  |
| `argument7` | ✅ | GM:S 1.0.98 |  |
| `argument8` | ✅ | GM:S 1.0.98 |  |
| `argument9` | ✅ | GM:S 1.0.98 |  |
| `argument_relative` | 🚫 | GM:S 1.0.98 |  |
| `async_load` | ✅ | GM:S 1.0.98 |  |
| `background_alpha` | ✅ | GM:S 1.0.98 | GM 2024.14.4.268 |
| `background_blend` | 🚫 | GM:S 1.0.98 | GM 2024.14.4.268 |
| `background_color` | ✅ | GM:S 1.0.98 | GM 2024.14.4.268 |
| `background_foreground` | 🚫 | GM:S 1.0.98 | GM 2024.14.4.268 |
| `background_height` | ✅ | GM:S 1.0.98 | GM 2024.14.4.268 |
| `background_hspeed` | ✅ | GM:S 1.0.98 | GM 2024.14.4.268 |
| `background_htiled` | 🚫 | GM:S 1.0.98 | GM 2024.14.4.268 |
| `background_index` | ✅ | GM:S 1.0.98 | GM 2024.14.4.268 |
| `background_showcolor` | 🚫 | GM:S 1.0.98 | GM 2024.14.4.268 |
| `background_visible` | ✅ | GM:S 1.0.98 | GM 2024.14.4.268 |
| `background_vspeed` | ✅ | GM:S 1.0.98 | GM 2024.14.4.268 |
| `background_vtiled` | 🚫 | GM:S 1.0.98 | GM 2024.14.4.268 |
| `background_width` | ✅ | GM:S 1.0.98 | GM 2024.14.4.268 |
| `background_x` | ✅ | GM:S 1.0.98 | GM 2024.14.4.268 |
| `background_xscale` | 🚫 | GM:S 1.0.98 | GM 2024.14.4.268 |
| `background_y` | ✅ | GM:S 1.0.98 | GM 2024.14.4.268 |
| `background_yscale` | 🚫 | GM:S 1.0.98 | GM 2024.14.4.268 |
| `bbox_bottom` | ✅ | GM:S 1.0.98 |  |
| `bbox_left` | ✅ | GM:S 1.0.98 |  |
| `bbox_right` | ✅ | GM:S 1.0.98 |  |
| `bbox_top` | ✅ | GM:S 1.0.98 |  |
| `browser_height` | 🚫 | GM:S 1.0.98 |  |
| `browser_width` | 🚫 | GM:S 1.0.98 |  |
| `caption_health` | 🚫 | GM:S 1.0.98 |  |
| `caption_lives` | 🚫 | GM:S 1.0.98 |  |
| `caption_score` | 🚫 | GM:S 1.0.98 |  |
| `current_day` | 🚫 | GM:S 1.0.98 |  |
| `current_hour` | 🚫 | GM:S 1.0.98 |  |
| `current_minute` | 🚫 | GM:S 1.0.98 |  |
| `current_month` | 🚫 | GM:S 1.0.98 |  |
| `current_second` | 🚫 | GM:S 1.0.98 |  |
| `current_time` | ✅ | GM:S 1.0.98 |  |
| `current_weekday` | 🚫 | GM:S 1.0.98 |  |
| `current_year` | 🚫 | GM:S 1.0.98 |  |
| `cursor_sprite` | 🚫 | GM:S 1.0.98 |  |
| `depth` | ✅ | GM:S 1.0.98 |  |
| `direction` | ✅ | GM:S 1.0.98 |  |
| `error_last` | 🚫 | GM:S 1.0.98 |  |
| `error_occurred` | 🚫 | GM:S 1.0.98 |  |
| `event_action` | 🚫 | GM:S 1.0.98 |  |
| `event_number` | 🚫 | GM:S 1.0.98 |  |
| `event_object` | 🚫 | GM:S 1.0.98 |  |
| `event_type` | 🚫 | GM:S 1.0.98 |  |
| `fps` | ✅ | GM:S 1.0.98 |  |
| `friction` | ✅ | GM:S 1.0.98 |  |
| `game_id` | 🚫 | GM:S 1.0.98 |  |
| `gamemaker_pro` | 🚫 | GM:S 1.0.98 |  |
| `gamemaker_registered` | 🚫 | GM:S 1.0.98 |  |
| `gravity` | ✅ | GM:S 1.0.98 |  |
| `gravity_direction` | ✅ | GM:S 1.0.98 |  |
| `health` | ✅ | GM:S 1.0.98 |  |
| `hspeed` | ✅ | GM:S 1.0.98 |  |
| `id` | ✅ | GM:S 1.0.98 |  |
| `image_alpha` | ✅ | GM:S 1.0.98 |  |
| `image_angle` | ✅ | GM:S 1.0.98 |  |
| `image_blend` | ✅ | GM:S 1.0.98 |  |
| `image_index` | ✅ | GM:S 1.0.98 |  |
| `image_number` | ✅ | GM:S 1.0.98 |  |
| `image_single` | ✅ | GM:S 1.0.98 |  |
| `image_speed` | ✅ | GM:S 1.0.98 |  |
| `image_xscale` | ✅ | GM:S 1.0.98 |  |
| `image_yscale` | ✅ | GM:S 1.0.98 |  |
| `instance_count` | 🚫 | GM:S 1.0.98 |  |
| `instance_id` | 🚫 | GM:S 1.0.98 |  |
| `keyboard_key` | ✅ | GM:S 1.0.98 |  |
| `keyboard_lastchar` | ✅ | GM:S 1.0.98 |  |
| `keyboard_lastkey` | ✅ | GM:S 1.0.98 |  |
| `keyboard_string` | 🚫 | GM:S 1.0.98 |  |
| `lives` | ✅ | GM:S 1.0.98 |  |
| `mask_index` | ✅ | GM:S 1.0.98 |  |
| `mouse_button` | 🚫 | GM:S 1.0.98 |  |
| `mouse_lastbutton` | 🚫 | GM:S 1.0.98 |  |
| `mouse_x` | 🚫 | GM:S 1.0.98 |  |
| `mouse_y` | 🚫 | GM:S 1.0.98 |  |
| `object_index` | ✅ | GM:S 1.0.98 |  |
| `os_device` | 🚫 | GM:S 1.0.98 |  |
| `os_type` | ✅ | GM:S 1.0.98 |  |
| `os_version` | 🚫 | GM:S 1.0.98 |  |
| `path_endaction` | ✅ | GM:S 1.0.98 |  |
| `path_index` | ✅ | GM:S 1.0.98 |  |
| `path_orientation` | ✅ | GM:S 1.0.98 |  |
| `path_position` | ✅ | GM:S 1.0.98 |  |
| `path_positionprevious` | ✅ | GM:S 1.0.98 |  |
| `path_scale` | ✅ | GM:S 1.0.98 |  |
| `path_speed` | ✅ | GM:S 1.0.98 |  |
| `persistent` | ✅ | GM:S 1.0.98 |  |
| `phy_angular_velocity` | 🚫 | GM:S 1.0.98 |  |
| `phy_bullet` | 🚫 | GM:S 1.0.98 |  |
| `phy_col_normal_x` | 🚫 | GM:S 1.0.98 |  |
| `phy_col_normal_y` | 🚫 | GM:S 1.0.98 |  |
| `phy_collision_points` | 🚫 | GM:S 1.0.98 |  |
| `phy_collision_x` | 🚫 | GM:S 1.0.98 |  |
| `phy_collision_y` | 🚫 | GM:S 1.0.98 |  |
| `phy_com_x` | 🚫 | GM:S 1.0.98 |  |
| `phy_com_y` | 🚫 | GM:S 1.0.98 |  |
| `phy_dynamic` | 🚫 | GM:S 1.0.98 |  |
| `phy_frozen` | 🚫 | GM:S 1.0.98 | GM:S 1.0.198 |
| `phy_inertia` | 🚫 | GM:S 1.0.98 |  |
| `phy_linear_velocity_x` | 🚫 | GM:S 1.0.98 |  |
| `phy_linear_velocity_y` | 🚫 | GM:S 1.0.98 |  |
| `phy_mass` | 🚫 | GM:S 1.0.98 |  |
| `phy_position_x` | 🚫 | GM:S 1.0.98 |  |
| `phy_position_y` | 🚫 | GM:S 1.0.98 |  |
| `phy_rotation` | 🚫 | GM:S 1.0.98 |  |
| `phy_sleeping` | 🚫 | GM:S 1.0.98 |  |
| `phy_speed_x` | 🚫 | GM:S 1.0.98 |  |
| `phy_speed_y` | 🚫 | GM:S 1.0.98 |  |
| `program_directory` | 🚫 | GM:S 1.0.98 |  |
| `room` | ✅ | GM:S 1.0.98 |  |
| `room_caption` | 🚫 | GM:S 1.0.98 |  |
| `room_first` | ✅ | GM:S 1.0.98 |  |
| `room_height` | ✅ | GM:S 1.0.98 |  |
| `room_last` | 🚫 | GM:S 1.0.98 |  |
| `room_persistent` | ✅ | GM:S 1.0.98 |  |
| `room_speed` | ✅ | GM:S 1.0.98 |  |
| `room_width` | ✅ | GM:S 1.0.98 |  |
| `score` | ✅ | GM:S 1.0.98 |  |
| `secure_mode` | 🚫 | GM:S 1.0.98 | GM 2024.14.4.268 |
| `show_health` | 🚫 | GM:S 1.0.98 |  |
| `show_lives` | 🚫 | GM:S 1.0.98 |  |
| `show_score` | 🚫 | GM:S 1.0.98 |  |
| `solid` | ✅ | GM:S 1.0.98 |  |
| `speed` | ✅ | GM:S 1.0.98 |  |
| `sprite_height` | ✅ | GM:S 1.0.98 |  |
| `sprite_index` | ✅ | GM:S 1.0.98 |  |
| `sprite_width` | ✅ | GM:S 1.0.98 |  |
| `sprite_xoffset` | ✅ | GM:S 1.0.98 |  |
| `sprite_yoffset` | ✅ | GM:S 1.0.98 |  |
| `temp_directory` | 🚫 | GM:S 1.0.98 |  |
| `timeline_index` | ✅ | GM:S 1.0.98 |  |
| `timeline_position` | ✅ | GM:S 1.0.98 |  |
| `timeline_speed` | ✅ | GM:S 1.0.98 |  |
| `transition_kind` | 🚫 | GM:S 1.0.98 |  |
| `transition_steps` | 🚫 | GM:S 1.0.98 |  |
| `view_angle` | ✅ | GM:S 1.0.98 |  |
| `view_current` | ✅ | GM:S 1.0.98 |  |
| `view_enabled` | 🚫 | GM:S 1.0.98 |  |
| `view_hborder` | ✅ | GM:S 1.0.98 |  |
| `view_hport` | ✅ | GM:S 1.0.98 |  |
| `view_hspeed` | ✅ | GM:S 1.0.98 |  |
| `view_hview` | ✅ | GM:S 1.0.98 |  |
| `view_object` | ✅ | GM:S 1.0.98 |  |
| `view_vborder` | ✅ | GM:S 1.0.98 |  |
| `view_visible` | ✅ | GM:S 1.0.98 |  |
| `view_vspeed` | ✅ | GM:S 1.0.98 |  |
| `view_wport` | ✅ | GM:S 1.0.98 |  |
| `view_wview` | ✅ | GM:S 1.0.98 |  |
| `view_xport` | ✅ | GM:S 1.0.98 |  |
| `view_xview` | ✅ | GM:S 1.0.98 |  |
| `view_yport` | ✅ | GM:S 1.0.98 |  |
| `view_yview` | ✅ | GM:S 1.0.98 |  |
| `visible` | ✅ | GM:S 1.0.98 |  |
| `vspeed` | ✅ | GM:S 1.0.98 |  |
| `working_directory` | ✅ | GM:S 1.0.98 |  |
| `x` | ✅ | GM:S 1.0.98 |  |
| `xprevious` | ✅ | GM:S 1.0.98 |  |
| `xstart` | ✅ | GM:S 1.0.98 |  |
| `y` | ✅ | GM:S 1.0.98 |  |
| `yprevious` | ✅ | GM:S 1.0.98 |  |
| `ystart` | ✅ | GM:S 1.0.98 |  |
| `argument_count` | ✅ | GM:S 1.0.114 |  |
| `display_aa` | 🚫 | GM:S 1.0.198 |  |
| `os_browser` | 🚫 | GM:S 1.0.198 |  |
| `phy_active` | 🚫 | GM:S 1.0.198 |  |
| `phy_fixed_rotation` | 🚫 | GM:S 1.0.198 |  |
| `phy_kinematic` | 🚫 | GM:S 1.0.198 |  |
| `timeline_loop` | ✅ | GM:S 1.0.198 |  |
| `timeline_running` | ✅ | GM:S 1.0.198 |  |
| `debug_mode` | ✅ | GM:S 1.1.622 |  |
| `delta_time` | 🚫 | GM:S 1.1.622 |  |
| `game_guid` | 🚫 | GM:S 1.1.622 | GM 2024.14.4.268 |
| `view_surface_id` | 🚫 | GM:S 1.1.622 |  |
| `webgl_enabled` | 🚫 | GM:S 1.1.622 |  |
| `fps_real` | 🚫 | GM:S 1.1.1130 |  |
| `phy_position_xprevious` | 🚫 | GM:S 1.2.1135 |  |
| `phy_position_yprevious` | 🚫 | GM:S 1.2.1135 |  |
| `phy_speed` | 🚫 | GM:S 1.2.1135 |  |
| `application_surface` | ✅ | GM:S 1.3.1307 |  |
| `iap_data` | 🚫 | GM:S 1.3.1307 |  |
| `background_colour` | ✅ | WAD Version 14 | GM 2024.14.4.268 |
| `background_showcolour` | 🚫 | WAD Version 14 | GM 2024.14.4.268 |
| `game_display_name` | 🚫 | WAD Version 14 |  |
| `game_project_name` | 🚫 | WAD Version 14 |  |
| `game_save_id` | 🚫 | WAD Version 14 |  |
| `phy_angular_damping` | 🚫 | WAD Version 14 |  |
| `phy_linear_damping` | 🚫 | WAD Version 14 |  |
| `pointer_invalid` | 🚫 | WAD Version 14 |  |
| `pointer_null` | 🚫 | WAD Version 14 |  |
| `undefined` | ✅ | WAD Version 14 |  |
| `layer` | ✅ | WAD Version 15 |  |
| `null` | 🚫 | WAD Version 15 | WAD Version 16 |
| `view_camera` | ✅ | WAD Version 15 |  |
| `event_data` | 🚫 | WAD Version 16 |  |
| `font_texture_page_size` | 🚫 | WAD Version 16 |  |
| `NaN` | 🚫 | GM 2024.14.4.268 |  |
| `audio_bus_main` | 🚫 | GM 2024.14.4.268 |  |
| `cache_directory` | 🚫 | GM 2024.14.4.268 |  |
| `collision_space` | 🚫 | GM 2024.14.4.268 |  |
| `drawn_by_sequence` | 🚫 | GM 2024.14.4.268 |  |
| `in_collision_tree` | 🚫 | GM 2024.14.4.268 |  |
| `in_sequence` | 🚫 | GM 2024.14.4.268 |  |
| `infinity` | 🚫 | GM 2024.14.4.268 |  |
| `managed` | 🚫 | GM 2024.14.4.268 |  |
| `on_ui_layer` | 🚫 | GM 2024.14.4.268 |  |
| `rollback_api_server` | 🚫 | GM 2024.14.4.268 |  |
| `rollback_confirmed_frame` | 🚫 | GM 2024.14.4.268 |  |
| `rollback_current_frame` | 🚫 | GM 2024.14.4.268 |  |
| `rollback_event_id` | 🚫 | GM 2024.14.4.268 |  |
| `rollback_event_param` | 🚫 | GM 2024.14.4.268 |  |
| `rollback_game_running` | 🚫 | GM 2024.14.4.268 |  |
| `sequence_instance` | 🚫 | GM 2024.14.4.268 |  |
| `wallpaper_config` | 🚫 | GM 2024.14.4.268 |  |
| `wallpaper_subscription_data` | 🚫 | GM 2024.14.4.268 |  |

